...
```{r rcbd_dfw}
rcbd_dfw <- rcbd_df %>%
  mutate(rep = factor(rep), #to treat "rep" i.e., "block" as categorical variable
         nrate_kgha = factor(nrate_kgha), #to treat "nrate_kgha" as categorical variable
         krate_kgha = factor(krate_kgha) #to treat "krate_kgha" as categorical variable
         ) %>%
  mutate(trtname = paste0(nrate_kgha,"+",krate_kgha)) #to combine the treatments and treat as factors
...
ggplot(rcbd_dfw, aes(x = nrate_kgha, 
  ...
  geom_boxplot() +
  geom_jitter() +
  facet_grid(.~krate_kgha) + #creates plots for individual plots to facet by
  theme(legend.position = "none")
```

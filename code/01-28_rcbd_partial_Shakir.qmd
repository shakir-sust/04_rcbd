...
```{r rcbd_dfw, warning=FALSE, message=FALSE}
...
```{r nk boxplot, warning=FALSE, message=FALSE}
ggplot(rcbd_dfw, aes(x = nrate_kgha, 
                    y = yield_kgha,
                    color = nrate_kgha)) +
  geom_boxplot() +
...
```{r checking for residual independence, warning=FALSE, message=FALSE}
...
```{r checing residual homoscedasticity, warning=FALSE, message=FALSE}
ggplot(rcbd_resid, aes(x=.fitted, y=.studresid))+
  geom_hline(yintercept = 0, color="red")+
  geom_point(shape = 21,
             fill = "purple", 
...
```{r checking residual outliers using fitted vs. residual plot, warning=FALSE, message=FALSE}
ggplot(rcbd_resid, aes(x=.fitted, y=.studresid))+
  geom_hline(yintercept = 0, color="red")+
  geom_point(shape = 21,
             fill = "purple", 
...
rcbd_means_nk <- emmeans(rcbd_mod, ~nrate_kgha | krate_kgha) #We are asking  for all the Nitrogen rates inside each one of the Potassium rates
...
rcbd_means_kn <- emmeans(rcbd_mod, ~krate_kgha | nrate_kgha) #We are asking the Potassium rates inside each one of the Nitrogen rates
...
```{r selected pwc, warning=FALSE, message=FALSE}
...
```{r rcbd final plot, warning=FALSE, message=FALSE}
...
ggplot() +
  ...
  geom_label(data = rcbd_cld_selected,
             aes(x = trtname, #We want to see the nitrogen rates in the x axis for all levels of pottasium rates
                 y = emmean,
                 label = letter
                 ),
             size = 3,
...
ggplot() +
  ...
  geom_boxplot(data = rcbd_dfw,
               aes(x = krate_kgha, y = yield_kgha,
                   fill = krate_kgha )) + #We want to see the nitrogen rate in the x axis for all levels of potassium rates
  geom_jitter() +
  theme(legend.position = "none") +
  # Adding letters
  geom_label(data = rcbd_cld_nk,
